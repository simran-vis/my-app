...
const ServiceCard = ({ icon, title, description, link }) => (
  <div className="flex flex-col items-center text-center p-4 sm:p-6 bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 w-36 sm:w-56 md:w-60">
    <div className="text-blue-500 text-3xl sm:text-4xl mb-2">{icon}</div>
    ...
    <a href={link} className="mt-2 text-blue-500 hover:underline text-xs sm:text-sm">
      Learn More
    </a>
  </div>
);
...
const ServiceSection = () => {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-8 w-full max-w-6xl mx-auto py-8 px-4 justify-items-center">
      <ServiceCard
        icon={<FaTruck />}
        title="Free Delivery"
        description="Free shipping on orders over $50"
